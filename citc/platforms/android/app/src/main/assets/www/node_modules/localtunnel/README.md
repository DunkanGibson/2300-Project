...
localtunnel exposes your 10.0.2.2 to the world for easy testing and sharing! No need to mess with DNS or deploy just to have others test out your changes.
...
* `--local-host` proxy to a hostname other than 10.0.2.2
...
* `local_host` Proxy to this hostname instead of `10.0.2.2`. This will also cause the `Host` header to be re-written to this value in proxied requests.
