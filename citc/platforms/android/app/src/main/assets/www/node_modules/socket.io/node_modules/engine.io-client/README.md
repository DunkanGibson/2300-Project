...
1. write your app code
    ...
    ```js
    var socket = require('engine.io-client')('ws://10.0.2.2');
    socket.on('open', function(){
      socket.on('message', function(data){});
      socket.on('close', function(){});
    });
...
<script src="/path/to/engine.io.js"></script>
<script>
  var socket = new eio.Socket('ws://10.0.2.2/');
...
var socket = require('engine.io-client')('ws://10.0.2.2', opts);
socket.on('open', function(){
  socket.on('message', function(data){});
  socket.on('close', function(){});
});
